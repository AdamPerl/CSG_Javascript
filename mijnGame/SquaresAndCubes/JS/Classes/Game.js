...
class Game {
  constructor(Player, Game_Mode) {
    ...
    this.Modifier = 0.0001; // make everything this times harder each score
    this.Scaling = null;
    this.Player = Player;
    this.Enemies = [];
    this.EnemyLootTable = [
      { type: "Rectangle", Weight: 10 },
      { type: "Beam", Weight: 1 },
  ...
  Handle_Frame() {
    ...
    if (Math.random() < (0.1 * (1+this.Modifier))) {
  ...
  Spawn_Random_Enemy() {
    ...
    if (Selected_Type == "Rectangle") {
      this.Enemies.push(new Enemy(Selected_Type, Math.random() * windowWidth, Math.random() * windowHeight, Math.random() * 50 + 50));
    }

    if (Selected_Type == "Beam") {
      this.Enemies.push(new Enemy(Selected_Type, Math.random() * windowHeight, null, 30, null));
    }
